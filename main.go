...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	distance := t.distance()
	meanSpeed := distance / MInKm / (t.Duration.Minutes() / MinInHours)
	return meanSpeed
}
...
// Формула расчета:
// ((18 * средняя_скорость_в_км/ч + 1.79) * вес_спортсмена_в_кг /
// / м_в_км * время_тренировки_в_часах * мин_в_часе)
// Это переопределенный метод Calories() из Training.
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	calories := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return calories
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	calories := (((CaloriesWeightMultiplier*w.Weight + math.Pow(w.meanSpeed()*KmHInMsec, 2)/float64((w.Height/CmInM))) * CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours() * MinInHours)
	return calories
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	meanSwimmingSpeed := float64(s.LengthPool*s.CountPool/MInKm) / s.Duration.Hours()
